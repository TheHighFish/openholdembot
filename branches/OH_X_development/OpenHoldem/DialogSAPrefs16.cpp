...
BOOL CDlgSAPrefs16::OnInitDialog()
{
	...
	_rebuy_condition_no_cards_Button.SetCheck(theApp.p_preferences()->rebuy_condition_no_cards());
	_rebuy_condition_change_in_handnumber_Button.SetCheck(theApp.p_preferences()->rebuy_condition_change_in_handnumber());
	_rebuy_condition_heuristic_check_for_occlusion_Button.SetCheck(theApp.p_preferences()->rebuy_condition_heuristic_check_for_occlusion());

	text.Format("%d", theApp.p_preferences()->rebuy_minimum_time_to_next_try());
	...
	_rebuy_minimum_time_to_next_try_Spin.SetPos(theApp.p_preferences()->rebuy_minimum_time_to_next_try());
	_rebuy_minimum_time_to_next_try_Spin.SetBuddy(&_rebuy_minimum_time_to_next_try_Edit);

	_rebuy_script_Edit.SetWindowText(theApp.p_preferences()->rebuy_script());
...
void CDlgSAPrefs16::OnOK()
{
	...
	theApp.p_preferences()->SetValue(k_prefs_rebuy_condition_no_cards, _rebuy_condition_no_cards_Button.GetCheck() == true);
	theApp.p_preferences()->SetValue(k_prefs_rebuy_condition_change_in_handnumber, _rebuy_condition_change_in_handnumber_Button.GetCheck() == true);
	theApp.p_preferences()->SetValue(k_prefs_rebuy_condition_heuristic_check_for_occlusion, _rebuy_condition_heuristic_check_for_occlusion_Button.GetCheck() == true);
	...
	theApp.p_preferences()->SetValue(k_prefs_rebuy_minimum_time_to_next_try, strtoul(text.GetString(), 0, 10));

	_rebuy_script_Edit.GetWindowText(text);
	theApp.p_preferences()->SetValue(k_prefs_rebuy_script, text);

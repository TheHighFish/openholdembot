...
{
	unsigned int MinimumTimeDifference = theApp.p_preferences()->rebuy_minimum_time_to_next_try();
	...
	{
		write_log(theApp.p_preferences()->debug_rebuy(), "[CRebuyManagement] MinimumDelayElapsed(): false\n");
		return false;
	}
	return true;
}
...
{
	if (!theApp.p_preferences()->rebuy_condition_change_in_handnumber()) 
	{
		return true;
	}
	...
	write_log(theApp.p_preferences()->debug_rebuy(), "[CRebuyManagement] ChangeInHandNumber(): false\n");
	return false;
}

bool CRebuyManagement::NoCards()
{
	if (!theApp.p_preferences()->rebuy_condition_no_cards()) 
	{
		return true;
	}
	...
	write_log(theApp.p_preferences()->debug_rebuy(), "[CRebuyManagement] No_Cards: false: we hold cards.\n");	
	return false;
}

bool CRebuyManagement::OcclusionCheck() {
	if (!theApp.p_preferences()->rebuy_condition_heuristic_check_for_occlusion()) {
		return true;
	}
  COcclusionCheck occlusioncheck;
	if (occlusioncheck.UserBalanceOccluded()) 	{
		write_log(theApp.p_preferences()->debug_rebuy(), "[CRebuyManagement] OcclusionCheck: false (occluded)\n");
		return false;
	}
	return true;
}
...
bool CRebuyManagement::RebuyPossible()
{
	if (MinimumDelayElapsed()
		...
		&& OcclusionCheck())
	{
		write_log(theApp.p_preferences()->debug_rebuy(), "[CRebuyManagement] RebuyPossible: true\n");
		return true;
	}
	else
	{
		write_log(theApp.p_preferences()->debug_rebuy(), "[CRebuyManagement] RebuyPossible: false\n");
		return false;
	}
}
...
{
	write_log(theApp.p_preferences()->debug_rebuy(), "[CRebuyManagement] TryToRebuy()\n");
	if (RebuyPossible())
	{
...
void CRebuyManagement::ExecuteRebuyScript()
{
	...
	write_log(theApp.p_preferences()->debug_rebuy(), "[CRebuyManagement] ExecuteRebuyScript");
	CString Casino;
	...
	CString RebuyScript = theApp.p_preferences()->rebuy_script();
	CString CommandLine;
	...
	write_log(theApp.p_preferences()->debug_rebuy(), "[CRebuyManagement] command line: %s\n", CommandLine);
	...
	else
	{
		...
		write_log(theApp.p_preferences()->debug_rebuy(), ErrorMessage.GetBuffer());

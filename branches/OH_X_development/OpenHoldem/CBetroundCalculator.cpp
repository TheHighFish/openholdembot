...
void CBetroundCalculator::OnNewHeartbeat()
{
	...
	write_log(theApp.p_preferences()->debug_alltherest(), "[CBetroundCalculator] _betround_previous_heartbeat = %i\n",
    ...
    if (p_table_state->_common_cards[4].IsKnownCard()) {
		...
		write_log(theApp.p_preferences()->debug_alltherest(), "[CBetroundCalculator] Animation going on\n");
		if (_betround_previous_heartbeat == k_undefined)
		{
			_betround = k_betround_preflop;
		}
	}
	write_log(theApp.p_preferences()->debug_alltherest(), "[CBetroundCalculator] _betround = %i\n",
		_betround);
}
...
{ 
	...
	write_log(theApp.p_preferences()->debug_alltherest(), "[CBetroundCalculator] IsNewBetround() = %s\n",
		Bool2CString(result));
	return result; 
}
...
BOOL CDlgSAPrefs18::OnInitDialog()
{
	...
	if (theApp.p_preferences()->lazy_scraping_when_to_scrape() == k_lazy_scraping_myturn)
	{
		_scrape_when_my_turn_button.SetCheck(true);
	}
	else if (theApp.p_preferences()->lazy_scraping_when_to_scrape() == k_lazy_scraping_cards)
	{
...
void CDlgSAPrefs18::OnOK()
{
	...
	{
		theApp.p_preferences()->SetValue(k_prefs_lazy_scraping_when_to_scrape, k_lazy_scraping_cards);
	}
	else if (_scrape_when_my_turn_button.GetCheck())
	{
		theApp.p_preferences()->SetValue(k_prefs_lazy_scraping_when_to_scrape, k_lazy_scraping_myturn);
	}
	else
	{
		// Default: k_lazy_scraping_always
		theApp.p_preferences()->SetValue(k_prefs_lazy_scraping_when_to_scrape, k_lazy_scraping_always);
	}
	CSAPrefsSubDlg::OnOK();
}

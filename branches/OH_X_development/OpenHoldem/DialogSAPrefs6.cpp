...
BOOL CDlgSAPrefs6::OnInitDialog()
{
	...
	m_pt_ip.SetWindowText(theApp.p_preferences()->pt_ip_addr().GetString());
	m_pt_port.SetWindowText(theApp.p_preferences()->pt_port().GetString());
	m_pt_user.SetWindowText(theApp.p_preferences()->pt_user().GetString());
	m_pt_pass.SetWindowText(theApp.p_preferences()->pt_pass().GetString());
	m_pt_dbname.SetWindowText(theApp.p_preferences()->pt_dbname().GetString());
...
void CDlgSAPrefs6::OnOK()
{
	...
	theApp.p_preferences()->SetValue(k_prefs_pt_ip_addr, text);

	m_pt_port.GetWindowText(text);
	theApp.p_preferences()->SetValue(k_prefs_pt_port, text);

	m_pt_user.GetWindowText(text);
	theApp.p_preferences()->SetValue(k_prefs_pt_user, text);

	m_pt_pass.GetWindowText(text);
	theApp.p_preferences()->SetValue(k_prefs_pt_pass, text);

	m_pt_dbname.GetWindowText(text);
	theApp.p_preferences()->SetValue(k_prefs_pt_dbname, text);
...
void CDlgSAPrefs6::OnBnClickedPtTest() {
	...
	if (dbname.IsEmpty()) {
		...
		write_log(theApp.p_preferences()->debug_pokertracker(), "[PokerTracker] Test: PostgreSQL DB Name not set ! Default : It will be set to '%s'\n", user);
	...
	if (PQstatus(pgconn) == CONNECTION_OK) {
		write_log(theApp.p_preferences()->debug_pokertracker(), "[PokerTracker] Test: PostgreSQL DB opened successfully <%s/%s/%s>\n", ip_addr, port, dbname);
		if (PQisthreadsafe()) {
			write_log(theApp.p_preferences()->debug_pokertracker(), "[PokerTracker] Test: PostgreSQL library is thread safe.\n\n");
			OH_MessageBox_Interactive("PostgreSQL DB opened successfully", "Success", MB_OK);
		} else {
			write_log(theApp.p_preferences()->debug_pokertracker(), "[PokerTracker] Test: PostgreSQL library is *NOT* thread safe!  This is a problem!\n\n");
		...
		write_log(theApp.p_preferences()->debug_pokertracker(), "[PokerTracker] Test: ERROR opening PostgreSQL DB: %s\n\n", PQerrorMessage(pgconn));

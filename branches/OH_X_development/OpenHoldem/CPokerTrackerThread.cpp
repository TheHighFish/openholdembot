	...
	const CString password, const CString DB_name)
{
	...
	write_log(theApp.p_preferences()->debug_pokertracker(), "[PokerTracker] Connection-string: %s\n", result);
	return result;
}
...
CPokerTrackerThread::CPokerTrackerThread()
{
	...
	_conn_str = CreateConnectionString(theApp.p_preferences()->pt_ip_addr(), 
		theApp.p_preferences()->pt_port(), theApp.p_preferences()->pt_user(), theApp.p_preferences()->pt_pass(), theApp.p_preferences()->pt_dbname());
...
void CPokerTrackerThread::StartThread()
{
	...
	if (_pt_thread == NULL)
	{
		...
		write_log(theApp.p_preferences()->debug_pokertracker(), "[PokerTracker] Started PokerTracker-thread.\n");
	}
	else
	{
		write_log(theApp.p_preferences()->debug_pokertracker(), "[PokerTracker] PokerTracker-thread already running.\n");
	}
}
...
void CPokerTrackerThread::StopThread()
{
	if (_pt_thread)
	{
		...
		write_log(theApp.p_preferences()->debug_pokertracker(), "[PokerTracker] Stopped Poker Tracker thread.\n");
	}
...
{
	return(theApp.p_preferences()->pt_ip_addr() != ""
		&& theApp.p_preferences()->pt_port()    != ""
		&& theApp.p_preferences()->pt_user()    != ""
		&& theApp.p_preferences()->pt_pass()    != ""
		&& theApp.p_preferences()->pt_dbname()  != "");


}

void CPokerTrackerThread::Connect(void) {
	write_log(theApp.p_preferences()->debug_pokertracker(), "[PokerTracker] Trying to open PostgreSQL DB...\n");
	if (!AllConnectionDataSpecified()) 	{
    write_log(theApp.p_preferences()->debug_pokertracker(), "[PokerTracker] Can't connect to DB. Vital data missing\n");
		return;
	}
	_conn_str = CreateConnectionString(theApp.p_preferences()->pt_ip_addr(), 
		theApp.p_preferences()->pt_port(), theApp.p_preferences()->pt_user(), theApp.p_preferences()->pt_pass(), theApp.p_preferences()->pt_dbname());
	...
	if (PQstatus(_pgconn) == CONNECTION_OK)	{
		write_log(theApp.p_preferences()->debug_pokertracker(), "[PokerTracker] PostgreSQL DB opened successfully <%s/%s/%s>\n", 
			theApp.p_preferences()->pt_ip_addr(), 
			theApp.p_preferences()->pt_port(), 
			theApp.p_preferences()->pt_dbname());

		_connected = true;
	}	else {
		write_log(theApp.p_preferences()->debug_pokertracker(), "[PokerTracker] ERROR opening PostgreSQL DB: %s\n\n", PQerrorMessage(_pgconn));
		PQfinish(_pgconn);
		_connected = false;
		_pgconn = NULL;
	}
...
{
  write_log(theApp.p_preferences()->debug_pokertracker(), "[PokerTracker] connected: %s\n",
    Bool2CString(_connected));
  write_log(theApp.p_preferences()->debug_pokertracker(), "[PokerTracker]PXStatus = %d (0 = CONNECTION_OK)\n",
    PQstatus(_pgconn));
...
bool CPokerTrackerThread::CheckIfNameExistsInDB(int chair)
{
	...
	write_log(theApp.p_preferences()->debug_pokertracker(), "[PokerTracker] CheckIfNameExistsInDB() chair = %i\n", chair);
	
	if (p_game_state->state((p_game_state->state_index()-1)&0xff)->m_player[chair].m_name_known == 0)
	{
		write_log(theApp.p_preferences()->debug_pokertracker(), "[PokerTracker] CheckIfNameExistsInDB() No name known for this chair\n");
		return false;
	}
	...
	write_log(theApp.p_preferences()->debug_pokertracker(), "[PokerTracker] CheckIfNameExistsInDB() Scraped name: [%s]\n", oh_scraped_name);

	if (NameLooksLikeBadScrape(oh_scraped_name))
	{
		write_log(theApp.p_preferences()->debug_pokertracker(), "[PokerTracker] CheckIfNameExistsInDB() Name looks like a bad scrape\n");
		return false;
	}
	...
	{
		write_log(theApp.p_preferences()->debug_pokertracker(), "[PokerTracker] CheckIfNameExistsInDB() Name is known and good\n");
		return true;
	}
	...
	{
		write_log(theApp.p_preferences()->debug_pokertracker(), "[PokerTracker] CheckIfNameExistsInDB() Name found in database\n");
		SetPlayerName(chair, true, best_name, oh_scraped_name);
		return true;
	}
	else
	{
		write_log(theApp.p_preferences()->debug_pokertracker(), "[PokerTracker] CheckIfNameExistsInDB() Name not found in database\n");
...
bool CPokerTrackerThread::CheckIfNameHasChanged(int chair)
{
	...
	write_log(theApp.p_preferences()->debug_pokertracker(), "[PokerTracker] Name changed for chair [%d] [%s] -> [%s]\n",
...
void CPokerTrackerThread::SetPlayerName(int chr, bool found, const char* pt_name, const char* scraped_name)
{
	...
	if (logResult)
	{
		write_log(theApp.p_preferences()->debug_pokertracker(), "[PokerTracker] SetPlayerName[%d]: Done. ptname[%s] scrapedName[%s]\n", chr, _player_data[chr].pt_name, _player_data[chr].scraped_name);
	}
}
...
double CPokerTrackerThread::UpdateStat(int m_chr, int stat)
{
	...
	try
	{
		...
		write_log(theApp.p_preferences()->debug_pokertracker(), 
	...
	{
		write_log(theApp.p_preferences()->debug_pokertracker(), "[PokerTracker] ERROR\n");
		write_log(theApp.p_preferences()->debug_pokertracker(), _T("\tCode = %08lx\n"), e.Error());
		write_log(theApp.p_preferences()->debug_pokertracker(), _T("\tCode meaning = %s\n"), e.ErrorMessage());
		_bstr_t bstrSource(e.Source());
		_bstr_t bstrDescription(e.Description());
		write_log(theApp.p_preferences()->debug_pokertracker(), _T("\tSource = %s\n"), (LPCTSTR) bstrSource);
		write_log(theApp.p_preferences()->debug_pokertracker(), _T("\tDescription = %s\n"), (LPCTSTR) bstrDescription);
		write_log(theApp.p_preferences()->debug_pokertracker(), _T("\tQuery = [%s]\n"), query);
	}
	
	updEnd = clock();
	duration = (int) ((double)(updEnd - updStart) / 1000);
	if (duration >= 3)
		write_log(theApp.p_preferences()->debug_pokertracker(), "[PokerTracker] Query time in seconds: [%d]\n", duration);
	...
	if (PQresultStatus(res) != PGRES_TUPLES_OK)
	{
		...
		case PGRES_COMMAND_OK:
			write_log(theApp.p_preferences()->debug_pokertracker(), "[PokerTracker] PGRES_COMMAND_OK: %s [%s]\n", PQerrorMessage(_pgconn), query);
			break;
		case PGRES_EMPTY_QUERY:
			write_log(theApp.p_preferences()->debug_pokertracker(), "[PokerTracker] PGRES_EMPTY_QUERY: %s [%s]\n", PQerrorMessage(_pgconn), query);
			break;
		case PGRES_BAD_RESPONSE:
			write_log(theApp.p_preferences()->debug_pokertracker(), "[PokerTracker] PGRES_BAD_RESPONSE: %s [%s]\n", PQerrorMessage(_pgconn), query);
			break;
		case PGRES_COPY_OUT:
			write_log(theApp.p_preferences()->debug_pokertracker(), "[PokerTracker] PGRES_COPY_OUT: %s [%s]\n", PQerrorMessage(_pgconn), query);
			break;
		case PGRES_COPY_IN:
			write_log(theApp.p_preferences()->debug_pokertracker(), "[PokerTracker] PGRES_COPY_IN: %s [%s]\n", PQerrorMessage(_pgconn), query);
			break;
		case PGRES_NONFATAL_ERROR:
			write_log(theApp.p_preferences()->debug_pokertracker(), "[PokerTracker] PGRES_NONFATAL_ERROR: %s [%s]\n", PQerrorMessage(_pgconn), query);
			break;
		case PGRES_FATAL_ERROR:
			write_log(theApp.p_preferences()->debug_pokertracker(), "[PokerTracker] PGRES_FATAL_ERROR: %s [%s]\n", PQerrorMessage(_pgconn), query);
			break;
		default:
			write_log(theApp.p_preferences()->debug_pokertracker(), "[PokerTracker] GENERIC ERROR: %s [%s]\n", PQerrorMessage(_pgconn), query);
			break;
		}
	}
	else
...
bool CPokerTrackerThread::QueryName(const char * query_name, const char * scraped_name, char * best_name)
{
	...
	{
		write_log(theApp.p_preferences()->debug_pokertracker(), "[PokerTracker] Postgres Query error:\n");
		write_log(theApp.p_preferences()->debug_pokertracker(), "[PokerTracker] \tCode = %08lx\n", e.Error());
		write_log(theApp.p_preferences()->debug_pokertracker(), "[PokerTracker] \tCode meaning = %s\n", e.ErrorMessage());
		_bstr_t bstrSource(e.Source());
		_bstr_t bstrDescription(e.Description());
		write_log(theApp.p_preferences()->debug_pokertracker(), "[PokerTracker] \tSource = %s\n", (LPCTSTR) bstrSource);
		write_log(theApp.p_preferences()->debug_pokertracker(), "[PokerTracker] \tDescription = %s\n", (LPCTSTR) bstrDescription);
		write_log(theApp.p_preferences()->debug_pokertracker(), "[PokerTracker] \tQuery = [%s]\n", query);
	}
...
bool CPokerTrackerThread::UpdateAllStats(int chr)
{
	...
	{
		write_log(theApp.p_preferences()->debug_pokertracker(), "[PokerTracker] UpdateAllStats: for chair [%d] is true.\n", chr);
		return true;
	}
	write_log(theApp.p_preferences()->debug_pokertracker(), "[PokerTracker] UpdateAllStats: for chair [%d] is false (Basic only).\n", chr);
	return false;
}
...
bool CPokerTrackerThread::SkipUpdateForChair(int chair)
{
	...
	{
		write_log(theApp.p_preferences()->debug_pokertracker(), "[PokerTracker] GetStatsForChair for chair [%d] had been skipped. Reason: [User sits in this chair]\n", chair);
		return true;
	}
	...
	if (hands > k_min_hands_for_slower_updates)
	{
		...
		{
			write_log(theApp.p_preferences()->debug_pokertracker(), "[PokerTracker] GetStatsForChair for chair [%d] had been skipped. Reason: [User has lots of hands]\n", chair);
			return true;
		}
	}
	return false;
...
void CPokerTrackerThread::GetStatsForChair(LPVOID pParam, int chair, int sleepTime)
{
  ...
  write_log(theApp.p_preferences()->debug_pokertracker(), "[PokerTracker] GetStatsForChair %i\n", chair);  
	if (pParent->CheckIfNameExistsInDB(chair) == false)
	{
		...
		write_log(theApp.p_preferences()->debug_pokertracker(), "[PokerTracker] GetStatsForChair[%d] had been skipped. Reason: [CheckName failed]\n", chair);
		return;
	}
	write_log(theApp.p_preferences()->debug_pokertracker(), "[PokerTracker] GetStatsForChair[%d] had been started.\n", chair);
			...
			for (int i=0; i<PT_DLL_GetNumberOfStats(); i++)
			{
				...
				if (_player_data[chair].found)
				{
					...
					if (pParent->CheckIfNameHasChanged(chair))
					{
						...
						*/
						write_log(theApp.p_preferences()->debug_pokertracker(), "[PokerTracker] Name changed for chair [%d] Stopping PT-symbol-lookup. \n", chair);
						return;
					}
					if (pParent->SkipUpdateCondition(i, chair))
					{
						...
						write_log(theApp.p_preferences()->debug_pokertracker(), "[PokerTracker] GetStatsForChair: Updating stats [%d] for chair [%d] had been skipped. Reason: [advanced/positional stats cycle [%d of %d]]\n", i, chair, pParent->GetSkippedUpdates(chair) , k_advanced_stat_update_every);
					}
					else
					{
						/* Update... */
						write_log(theApp.p_preferences()->debug_pokertracker(), "[PokerTracker] GetStatsForChair updating stats [%d] for chair [%d]...\n", i, chair);
				...
				else
				{
					...
					write_log(theApp.p_preferences()->debug_pokertracker(), "[PokerTracker] GetStatsForChair for chair [%d] had been skipped. Reason: [user not found (user stood up?)]\n", chair);
					return;
				}
			}
		}
...
{
	write_log(theApp.p_preferences()->debug_pokertracker(), "[PokerTracker] Updates for chair [%d][%s] had been completed. Total [%d] updated stats\n", chair, _player_data[chair].scraped_name, updatedCount);
}
...
UINT CPokerTrackerThread::PokertrackerThreadFunction(LPVOID pParam)
{
	...
	{
		iterStart = clock();
		write_log(theApp.p_preferences()->debug_pokertracker(), "[PokerTracker] PTthread iteration [%d] had started\n", ++iteration);
		if (!pParent->_connected)
		{
			pParent->Connect();
		}
		...
		write_log(theApp.p_preferences()->debug_pokertracker(), "[PokerTracker] Players count is [%d]\n", players);
				
		if (players < 2)
		{
			write_log(theApp.p_preferences()->debug_pokertracker(), "[PokerTracker] Not enough players to justify iteration...\n");
			write_log(theApp.p_preferences()->debug_pokertracker(), "[PokerTracker] For beginners: possible tablemap-problem?\n");
			write_log(theApp.p_preferences()->debug_pokertracker(), "[PokerTracker] Continuing anyway...\n");
		}
		...
		write_log(theApp.p_preferences()->debug_pokertracker(), "[PokerTracker] sleepTime set to %d\n", sleep_time);
		...
		write_log(theApp.p_preferences()->debug_pokertracker(), "[PokerTracker] PTthread iteration [%d] had ended, duration time in ms: [%d]\n", ++iteration, iterDurationMS);
		if (iterDurationMS <= 10000)
		{
			write_log(theApp.p_preferences()->debug_pokertracker(), "[PokerTracker] sleeping [%d] ms because iteration was too quick.\n", 10000 - iterDurationMS);
			if (LightSleep(10000 - iterDurationMS, pParent)) 
				break; 
		}
	}
	// Set event
	write_log(theApp.p_preferences()->debug_pokertracker(), "[PokerTracker] PokertrackerThreadFunction: outside while loop...\n");
...
{
	write_log(theApp.p_preferences()->debug_pokertracker(), "[PokerTracker] LightSleep: called with sleepTime[%d]\n", sleepTime);
	if ( sleepTime > 0)
	{
		int iterations = 20;
		...
		for (int i = 0; i < iterations; i++)
		{
			...
			{
				write_log(theApp.p_preferences()->debug_pokertracker(), "[PokerTracker] LightSleep: _m_stop_thread signal received\n");
				return 1;
			}
		}
	}

...
BOOL CDlgSAPrefs14::OnInitDialog()
{
	...
	if (theApp.p_preferences()->autoconnector_when_to_connect() == k_AutoConnector_Connect_Never)
	{
		_autoconnector_connect_never.SetCheck(true);
	}
	else if (theApp.p_preferences()->autoconnector_when_to_connect() == k_AutoConnector_Connect_Once)
	{
	...
	_autoconnector_close_when_table_disappears.SetCheck(theApp.p_preferences()->autoconnector_close_when_table_disappears());
...
void CDlgSAPrefs14::OnOK()
{
	...
	{
		theApp.p_preferences()->SetValue(k_prefs_autoconnector_when_to_connect, k_AutoConnector_Connect_Never);
	}
	else if (bool(_autoconnector_connect_once.GetCheck()) == true)
	{
		theApp.p_preferences()->SetValue(k_prefs_autoconnector_when_to_connect, k_AutoConnector_Connect_Once);
	}
	else // _autoconnector_connect_permanent.GetCheck() == true
	{
		theApp.p_preferences()->SetValue(k_prefs_autoconnector_when_to_connect, k_AutoConnector_Connect_Permanent);
	}

	theApp.p_preferences()->SetValue(k_prefs_autoconnector_close_when_table_disappears, _autoconnector_close_when_table_disappears.GetCheck());
	CSAPrefsSubDlg::OnOK();
}

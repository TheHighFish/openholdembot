...
void CAutoplayerFunctions::CalcPrimaryFormulas() {
  write_log(MAIN->p_preferences()->debug_formula(), "[CAutoplayerFunctions] CalcPrimaryFormulas()\n");
...
void CAutoplayerFunctions::CalcPrimaryFormulasOHScript() {
  write_log(MAIN->p_preferences()->debug_formula(), "[CAutoplayerFunctions] CalcPrimaryFormulasOHScript()\n");
  bool trace_needed = MAIN->p_preferences()->trace_enabled();
  write_log(MAIN->p_preferences()->debug_formula(), "[CAutoplayerFunctions] Trace enabled: %s\n", Bool2CString(MAIN->p_preferences()->trace_enabled()));
	for (int i=k_autoplayer_function_beep; i<=k_autoplayer_function_fold; i++) {
		...
		write_log(MAIN->p_preferences()->debug_formula(), "[CAutoplayerFunctions] Primary formulas; %s: %f\n", 
			k_standard_function_names[i], result);
	}
}

void CAutoplayerFunctions::CalcPrimaryFormulasOpenPPL() {
  write_log(MAIN->p_preferences()->debug_formula(), "[CAutoplayerFunctions] CalcPrimaryFormulasOpenPPL()\n");
  bool trace_needed = MAIN->p_preferences()->trace_enabled();
  write_log(MAIN->p_preferences()->debug_formula(), "[CAutoplayerFunctions] Trace enabled: %s\n", Bool2CString(MAIN->p_preferences()->trace_enabled()));
...
void CAutoplayerFunctions::TranslateOpenPPLDecisionToAutoplayerFunctions(double decision) {
  write_log(MAIN->p_preferences()->debug_formula(), "[CAutoplayerFunctions] TranslateOpenPPLDecisionToAutoplayerFunctions()\n");
...
void CAutoplayerFunctions::CalcSecondaryFormulas(void)
{
	for (int i=k_hopper_function_sitin; i<=k_standard_function_allin_on_betsize_balance_ratio; i++)
	{
		...
		write_log(MAIN->p_preferences()->debug_formula(), "[CAutoplayerFunctions] Secondary formulas; %s: %f\n", 
			k_standard_function_names[i], result);
	}
}

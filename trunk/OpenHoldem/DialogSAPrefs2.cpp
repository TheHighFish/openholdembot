...
BOOL CDlgSAPrefs2::OnInitDialog()
{
    ...
    if (p_Preferences->button_click_method() == BUTTON_SINGLECLICK)
        m_ButtonClicking.SelectString(0, "Single");
    else if (p_Preferences->button_click_method() == BUTTON_DOUBLECLICK)
        m_ButtonClicking.SelectString(0, "Double");

    m_AutoAP.SetCheck(p_Preferences->ap_auto() ? BST_CHECKED : BST_UNCHECKED);

    m_FocusDetect.SetCheck(p_Preferences->focus_detect() ? BST_CHECKED : BST_UNCHECKED);
...
void CDlgSAPrefs2::OnOK()
{
    ...
    p_Preferences->Set_frame_delay(strtoul(text.GetString(), 0, 10));
    ...
    p_Preferences->Set_swag_delay_1(strtoul(text.GetString(), 0, 10));
    ...
    p_Preferences->Set_swag_delay_2(strtoul(text.GetString(), 0, 10));
    ...
    p_Preferences->Set_swag_delay_3(strtoul(text.GetString(), 0, 10));

    if (m_TextSelection.GetCurSel()==0)
        p_Preferences->Set_text_selection_method(TEXTSEL_DOUBLECLICK);
    else if (m_TextSelection.GetCurSel()==1)
        p_Preferences->Set_text_selection_method(TEXTSEL_CLICKDRAG);

    if (m_TextDeletion.GetCurSel()==0)
        p_Preferences->Set_text_deletion_method(TEXTDEL_DELETE);
    else if (m_TextDeletion.GetCurSel()==1)
        p_Preferences->Set_text_deletion_method(TEXTDEL_BACKSPACE);

    if (m_BetConfirmation.GetCurSel()==0)
        p_Preferences->Set_bet_confirmation_method(BETCONF_ENTER);
    else if (m_BetConfirmation.GetCurSel()==1)
        p_Preferences->Set_bet_confirmation_method(BETCONF_CLICKBET);

    if (m_ButtonClicking.GetCurSel()==0)
        p_Preferences->Set_button_click_method(BUTTON_SINGLECLICK);
    else if (m_ButtonClicking.GetCurSel()==1)
        p_Preferences->Set_button_click_method(BUTTON_DOUBLECLICK);

    p_Preferences->Set_ap_auto(m_AutoAP.GetCheck()==BST_CHECKED ? true : false);
    p_Preferences->Set_focus_detect(m_FocusDetect.GetCheck()==BST_CHECKED ? true : false);

...
{		
	...
	{
		write_log(theApp.p_preferences()->debug_table_positioner(), "[CTablePositioner] PositionMyWindow() Going to tile %d windows...\n", _number_of_tables);
	...
	{
		write_log(theApp.p_preferences()->debug_table_positioner(), "[CTablePositioner] PositionMyWindow() Going to cascade %d windows...\n", _number_of_tables);
	...
	{
		// theApp.p_preferences()->table_positioner_options() == k_position_tables_never
		write_log(theApp.p_preferences()->debug_table_positioner(), "[CTablePositioner] PositionMyWindow() Not doing anything because of theApp.p_preferences()->\n");
	}
}
...
void CTablePositioner::PositionMyWindow(HWND *list_of_tables)
{
	...
	write_log(theApp.p_preferences()->debug_table_positioner(), "[CTablePositioner] PositionMyWindow() Connected to window %i\n", p_autoconnector->attached_hwnd());
	write_log(theApp.p_preferences()->debug_table_positioner(), "[CTablePositioner] PositionMyWindow() Table size: %ix%i\n", _table_size_x, _table_size_y);
	write_log(theApp.p_preferences()->debug_table_positioner(), "[CTablePositioner] PositionMyWindow() Desktop size: %ix%i\n", _desktop_rectangle.right, _desktop_rectangle.bottom); 
	...
	for (int i=0; i<_number_of_tables; i++)
	{
		...
		write_log(theApp.p_preferences()->debug_table_positioner(), "[CTablePositioner] PositionMyWindow() Trying neighbour-table %i\n", HWND_of_potential_neighbour_table);
...
bool CTablePositioner::TryLeftSideOfTable(HWND HWND_of_potential_neighbour_table)
{
	...
	write_log(theApp.p_preferences()->debug_table_positioner(), "[CTablePositioner] TryLeftSideOfTable() Neighbours position: %i, %i, %i, %i\n", 
	...
	write_log(theApp.p_preferences()->debug_table_positioner(), "[CTablePositioner] TryLeftSideOfTable() Trying right side of neighbour\n");
...
bool CTablePositioner::TryTopSideOfTable(HWND HWND_of_potential_neighbour_table)
{
	RECT position_of_potential_neigbour_table;
	GetWindowRect(HWND_of_potential_neighbour_table, &position_of_potential_neigbour_table);
	write_log(theApp.p_preferences()->debug_table_positioner(), "[CTablePositioner] TryTopSideOfTable() Trying bottom side of neighbour\n");
...
bool CTablePositioner::TryBottomRightPosition()
{
	...
	write_log(theApp.p_preferences()->debug_table_positioner(), "[CTablePositioner] TryBottomRightPosition()\n");
	return TryPosition(left_x, top_y);
}

bool CTablePositioner::TryPosition(int left_x, int top_y)
{
	write_log(theApp.p_preferences()->debug_table_positioner(), "[CTablePositioner] TryPosition() Trying position %i, %i\n",
		left_x, top_y);
	for (int i=0; i<_number_of_tables; i++)
	{
		...
		{
			write_log(theApp.p_preferences()->debug_table_positioner(), "[CTablePositioner] TryPosition() Candidate-position overlaps table\n");
			return false;
		}
	}
	...
	_new_left_x = left_x;
	_new_top_y  = top_y;
	write_log(theApp.p_preferences()->debug_table_positioner(), "[CTablePositioner] TryPosition() Found a free slot\n");
	write_log(theApp.p_preferences()->debug_table_positioner(), "[CTablePositioner] TryPosition() Going to move the table to %i, %i\n",
	...
	int top_y, HWND table_to_check_for_overlapping)
{
	...
	{
		write_log(theApp.p_preferences()->debug_table_positioner(), "[CTablePositioner] PotentialNewPositionOverlapsTable() Not a window, therefore no overlap\n");
		return false;
	}
	...
	if ((left_x < 0) || (top_y < 0))
	{
		// No good position
		// Treat it as overlap
		write_log(theApp.p_preferences()->debug_table_positioner(), "[CTablePositioner] PotentialNewPositionOverlapsTable() Out of desktop. No valid position\n");
		return true;
	}
	...
	{
		write_log(theApp.p_preferences()->debug_table_positioner(), "[CTablePositioner] PotentialNewPositionOverlapsTable() No overlap. Potential good position\n");
		// No overlap
		return false;
	}
	// No overlap so far
	...
	write_log(theApp.p_preferences()->debug_table_positioner(), "[CTablePositioner] PotentialNewPositionOverlapsTable() Position overlaps a table. Bad position\n");
	// Table overlaps
	return true;
}
...
{
	write_log(theApp.p_preferences()->debug_table_positioner(), "[CTablePositioner] MoveToTopLeft()\n");
	_new_left_x = 0;
	_new_top_y  = 0;
	MoveWindowToItsPosition();
}
...
{
	write_log(theApp.p_preferences()->debug_table_positioner(), "[CTablePositioner] MoveWindowToItsPosition()\n");
		...
		|| _table_size_y < 1 || _table_size_y > _desktop_rectangle.bottom)
	{
		...
		write_log(theApp.p_preferences()->debug_table_positioner(), "[CTablePositioner] Values out of sane range; probably not yet initialized\n");
		write_log(theApp.p_preferences()->debug_table_positioner(), "[CTablePositioner] _new_left_x   = %i\n", _new_left_x);
		write_log(theApp.p_preferences()->debug_table_positioner(), "[CTablePositioner] _new_top_y    = %i\n", _new_top_y);
		write_log(theApp.p_preferences()->debug_table_positioner(), "[CTablePositioner] _table_size_x = %i\n", _table_size_x);
		write_log(theApp.p_preferences()->debug_table_positioner(), "[CTablePositioner] _table_size_y = %i\n", _table_size_y);
		return;
	}
...
{
	if (!theApp.p_preferences()->table_positioner_always_keep_position()
		|| (p_autoconnector->attached_hwnd() == NULL))
	{
		write_log(theApp.p_preferences()->debug_table_positioner(), "[CTablePositioner] AlwaysKeepPositionIfEnabled() disabled or not connected\n");
		return;
	}
	RECT current_position;
	GetWindowRect(p_autoconnector->attached_hwnd(), &current_position);
	...
	{
		write_log(theApp.p_preferences()->debug_table_positioner(), "[CTablePositioner] AlwaysKeepPositionIfEnabled() position is good\n");
	}
	else
	{
		write_log(theApp.p_preferences()->debug_table_positioner(), "[CTablePositioner] AlwaysKeepPositionIfEnabled() restoring old position\n");
		MoveWindowToItsPosition();
	}
}
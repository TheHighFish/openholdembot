...
void CValidator::ValidateSingleRule() {
	...
	if (_heuristic && !theApp.p_preferences()->validator_use_heuristic_rules()) {
		return;
	}
	// Test to be executed?
	if (_precondition) { 
	  ...
	  if (!_postcondition ) { 
			if (_no_errors_this_heartbeat) {
				...
				if (theApp.p_preferences()->validator_shoot_replayframe_on_error()) {
					SYM->p_symbol_engine_replayframe_controller()->ShootReplayFrameIfNotYetDone();
				}
				_no_errors_this_heartbeat = false;
			}
			if (theApp.p_preferences()->validator_stop_on_error()) { 
...
void CValidator::ValidateGameState() {
	if (// Enabled, when it's my turn?
		  (theApp.p_preferences()->validator_enabled() && (SYM->p_symbol_engine_autoplayer()->ismyturn())) 

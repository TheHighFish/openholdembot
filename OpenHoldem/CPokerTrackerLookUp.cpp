...
const int CPokerTrackerLookup::GetSiteId()
{
  ...
  // http://www.maxinmontreal.com/forums/viewtopic.php?f=114&t=12158&p=108712#p108712
	...
	while (lookup!=end)
	{
		...
		{
			write_log(theApp.p_preferences()->debug_pokertracker(), "[PokerTrackerLookup] Site or network is: %s\n",
				supported_sitename);
			write_log(theApp.p_preferences()->debug_pokertracker(), "[PokerTrackerLookup] SiteID is %i\n",
				lookup->second);

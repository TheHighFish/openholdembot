...
CAutoConnector::~CAutoConnector()
{
	...
	write_log(theApp.p_preferences()->debug_autoconnector(), "[CAutoConnector] ~CAutoConnector() Marking table as not atached\n");
	p_sharedmem->MarkPokerWindowAsUnAttached();
	set_attached_hwnd(NULL);
	write_log(theApp.p_preferences()->debug_autoconnector(), "[CAutoConnector] ~CAutoConnector() Finished\n");
}
...
{
	write_log(theApp.p_preferences()->debug_autoconnector(), "[CAutoConnector] Check_TM_Against_All_Windows(..)\n");
...
BOOL CALLBACK EnumProcTopLevelWindowList(HWND hwnd, LPARAM lparam) 
{
	...
	write_log(theApp.p_preferences()->debug_autoconnector(), "[CAutoConnector] EnumProcTopLevelWindowList(..)\n");
	write_log(theApp.p_preferences()->debug_autoconnector(), "[CAutoConnector] Tablemap nr. %d\n", tablemap_index);
	...
	write_log(theApp.p_preferences()->debug_autoconnector(), "[CAutoConnector] EnumProcTopLevelWindowList(..) found a window candidate...\n");
	...
	if (Check_TM_Against_Single_Window(tablemap_index, hwnd, crect, title))
	{
		...
		{
			write_log(theApp.p_preferences()->debug_autoconnector(), "[CAutoConnector] Window candidate already served: [%d]\n", hwnd);
		}
		else
		{
			write_log(theApp.p_preferences()->debug_autoconnector(), "[CAutoConnector] Adding window candidate to the list: [%d]\n", hwnd);
...
void CAutoConnector::FailedToConnectBecauseNoWindowInList()
{
	...
	if (theApp.p_preferences()->autoconnector_when_to_connect() == k_AutoConnector_Connect_Once)
	{
...
{
	write_log(theApp.p_preferences()->debug_autoconnector(), "[CAutoConnector] Attempt to connect did fail\n");
	p_sharedmem->RememberTimeOfLastFailedAttemptToConnect();
	GoIntoPopupBlockingMode();
}
...
void CAutoConnector::GoIntoPopupBlockingMode()
{
	...
	write_log(theApp.p_preferences()->debug_autoconnector(), "[CAutoConnector] Not connected. Going into popup-blocking mode.\n");
...
bool CAutoConnector::Connect(HWND targetHWnd) {
	...
	write_log(theApp.p_preferences()->debug_autoconnector(), "[CAutoConnector] Connect(..)\n");

	ASSERT(_autoconnector_mutex->m_hObject != NULL); 
	write_log(theApp.p_preferences()->debug_autoconnector(), "[CAutoConnector] Locking autoconnector-mutex\n");
	if (!_autoconnector_mutex->Lock(500))	{
		write_log(theApp.p_preferences()->debug_autoconnector(), "[CAutoConnector] Could not grab mutex; early exit\n");
		return false; 
	}
  // Clear global list for holding table candidates
	g_tlist.RemoveAll();
	write_log(theApp.p_preferences()->debug_autoconnector(), "[CAutoConnector] Number of tablemaps loaded: %i\n",
	...
	for (int tablemap_index=0; tablemap_index<_p_tablemap_loader->NumberOfTableMapsLoaded(); tablemap_index++) {
		write_log(theApp.p_preferences()->debug_autoconnector(), "[CAutoConnector] Going to check TM nr. %d out of %d\n", 
  ...
  write_log(theApp.p_preferences()->debug_autoconnector(), "[CAutoConnector] Number of table candidates> %i\n", N);
	if (N == 0) {
		...
		if (SelectedItem == k_undefined) {
			...
			write_log(theApp.p_preferences()->debug_autoconnector(), "[CAutoConnector] Window [%d] selected\n", g_tlist[SelectedItem].hwnd);
			p_sharedmem->MarkPokerWindowAsAttached(g_tlist[SelectedItem].hwnd);
			write_log(theApp.p_preferences()->debug_autoconnector(), "[CAutoConnector] Window marked at shared memory\n");
			...
			write_log(theApp.p_preferences()->debug_autoconnector(), "[CAutoConnector] Selected tablemap: %s\n", tablemap_to_load);
			p_tablemap->LoadTablemap(tablemap_to_load);
			write_log(theApp.p_preferences()->debug_autoconnector(), "[CAutoConnector] Tablemap loaded\n");

			// Create bitmaps
			p_scraper->CreateBitmaps();
			write_log(theApp.p_preferences()->debug_autoconnector(), "[CAutoConnector] Scraper-bitmaps created\n");

			// Clear scraper fields
			p_table_state->Reset();
			write_log(theApp.p_preferences()->debug_autoconnector(), "[CAutoConnector] Table state cleared\n");

			// Reset symbols
			p_engine_container->ResetOnConnection();

			write_log(theApp.p_preferences()->debug_autoconnector(), "[CAutoConnector] ResetOnConnection executed (during connection)\n");
			write_log(theApp.p_preferences()->debug_autoconnector(), "[CAutoConnector] Going to continue with scraper output and scraper DLL\n");
	...
	write_log(theApp.p_preferences()->debug_autoconnector(), "[CAutoConnector] Unlocking autoconnector-mutex\n");
	_autoconnector_mutex->Unlock();
...
void CAutoConnector::LoadScraperDLL()
{
	...
	{
			write_log(theApp.p_preferences()->debug_autoconnector(), "[CAutoConnector] scraper.dll (%s) loaded, ProcessMessage and OverrideScraper found.\n", filename);
	}
}

void CAutoConnector::Disconnect() {
	write_log(theApp.p_preferences()->debug_autoconnector(), "[CAutoConnector] Disconnect()\n");
  ...
  CDlgScraperOutput::DestroyWindowSafely();
	...
	write_log(theApp.p_preferences()->debug_autoconnector(), "[CAutoConnector] Stopping autoplayer\n");
	...
	write_log(theApp.p_preferences()->debug_autoconnector(), "[CAutoConnector] Marking table as not attached\n");
	p_sharedmem->MarkPokerWindowAsUnAttached();
	...
	write_log(theApp.p_preferences()->debug_autoconnector(), "[CAutoConnector] ResetOnConnection executed (disconnection)\n");
	write_log(theApp.p_preferences()->debug_autoconnector(), "[CAutoConnector] Going to continue with window title\n");
	...
	// Close OH, when table disappears and leaving enabled in theApp.p_preferences()->
	if (theApp.p_preferences()->autoconnector_close_when_table_disappears()) {
		PostQuitMessage(0);
	}
	write_log(theApp.p_preferences()->debug_autoconnector(), "[CAutoConnector] Disconnect done\n");
}


int CAutoConnector::SelectTableMapAndWindow(int Choices)
{
	write_log(theApp.p_preferences()->debug_autoconnector(), "[CAutoConnector] SelectTableMapAndWindow(..)\n");
...
{
	write_log(theApp.p_preferences()->debug_autoconnector(), "[CAutoConnector] SelectTableMapAndWindowAutomatically(..)\n");
	for (int i=0; i<Choices; i++) 
	{
		if (!p_sharedmem->PokerWindowAttached(g_tlist[i].hwnd))
		{
			write_log(theApp.p_preferences()->debug_autoconnector(), "[CAutoConnector] Chosen (table, TM)-pair in list: %d\n", i);
			return i;
		}
	}
	// No appropriate table found
	write_log(theApp.p_preferences()->debug_autoconnector(), "[CAutoConnector] No appropriate table found.\n");
	return -1;
}
...
double CAutoConnector::TimeSincelast_failed_attempt_to_connect()
{
	...
	write_log(theApp.p_preferences()->debug_autoconnector(), "[CAutoConnector] TimeSincelast_failed_attempt_to_connect %f\n", _TimeSincelast_failed_attempt_to_connect);

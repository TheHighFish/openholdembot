...
void CSymbolEnginePokerTracker::ClearSeatStats(int chair, bool clearNameAndFound) {
	...
	write_log(theApp.p_preferences()->debug_pokertracker(), "[CSymbolEnginePokerTracker] ClearSeatStats() for chair %i\n", chair);
...
void CSymbolEnginePokerTracker::ClearAllStatsOfChangedPlayers() {
	write_log(theApp.p_preferences()->debug_pokertracker(), "[CSymbolEnginePokerTracker] Executing ClearAllStatsOfChangedPlayers()\n");
...
bool CSymbolEnginePokerTracker::EvaluateSymbol(const char *name, double *result, bool log /* = false */)
{
  FAST_EXIT_ON_OPENPPL_SYMBOLS(name);
	...
	if (!p_pokertracker_thread->IsConnected()) 	{
      ...
      write_log(theApp.p_preferences()->debug_pokertracker(), "[PokerTracker] Not yet seated or formula parsing.\n");
		} else {

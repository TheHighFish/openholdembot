...
double CSymbolEngineCallers::LastOrbitsLastRaisersBet() {
	// Not yet acted: 0
...
void CSymbolEngineCallers::CalculateRaisers() {
	_nopponentstruelyraising = 0;
    ...
    write_log(preferences.debug_symbolengine(), 
      "[CSymbolEngineCallers] No bet to call, therefore no raises\n");
		return;
	}
  ...
  write_log(preferences.debug_symbolengine(), "[CSymbolEngineCallers] Searching for raisers from chair %i to %i with a bet higher than %.2f\n",
    ...
    write_log(preferences.debug_symbolengine(), 
      "[CSymbolEngineCallers] chair %d bet %.2f\n",
    ...
    if (!p_table_state->_players[chair].HasAnyCards()) {
      write_log(preferences.debug_symbolengine(), 
        "[CSymbolEngineCallers] chair %d has no cards.\n", chair);
      continue;
    } else if (current_players_bet <= highest_bet) {
      write_log(preferences.debug_symbolengine(), 
        "[CSymbolEngineCallers] chair %d is not raising\n", chair);
      continue;
    } else if ((p_betround_calculator->betround() == kBetroundPreflop)
				&& (current_players_bet <= p_symbol_engine_tablelimits->bblind())) {
      write_log(preferences.debug_symbolengine(), 
        "[CSymbolEngineCallers] chair %d so-called \"blind raiser\". To be ignored.\n", chair);
      continue;
    }
    else if ((p_betround_calculator->betround() == kBetroundPreflop)
      && p_table_state->_players[chair].PostingBothBlinds()) {
      write_log(preferences.debug_symbolengine(), 
        "[CSymbolEngineCallers] chair %d is posting both blinds at once. To be ignored.\n", chair);
      continue;
    }
		highest_bet = current_players_bet;
    write_log(preferences.debug_symbolengine(), "[CSymbolEngineCallers] Opponent %i raising to %s\n",
  ...
  _lastraised[BETROUND] = _raischair;
	write_log(preferences.debug_symbolengine(), "[CSymbolEngineCallers] nopponentstruelyraising: %i\n", _nopponentstruelyraising);
	write_log(preferences.debug_symbolengine(), "[CSymbolEngineCallers] raischair: %i\n", _raischair);
}
...
int CSymbolEngineCallers::FirstPossibleRaiser() {
...
int CSymbolEngineCallers::LastPossibleRaiser() {
...
void CSymbolEngineCallers::CalculateNOpponentsCheckingBettingFolded()
{
	_nplayerscallshort  = 0;
	_nopponentsbetting  = 0;
	_nopponentsfolded   = 0;
...
double CSymbolEngineCallers::RaisersBet()
{
...
void CSymbolEngineCallers::CalculateFoldBits()
{
...
int CSymbolEngineCallers::LastRaised(const int round) {

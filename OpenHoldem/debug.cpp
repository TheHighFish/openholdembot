...
void start_log(void) {
	...
	CString fn = CFilenames::LogFilename();
	// Check, if file exists and size is too large
	struct stat file_stats = { 0 };
	if (stat(fn.GetString(), &file_stats) == 0)
	{
		unsigned long int max_file_size = 1E06 * theApp.p_preferences()->log_max_logsize();
...
int GenerateDump(EXCEPTION_POINTERS *pExceptionPointers)
{
	...
	hDumpFile = CreateFile(CFilenames::MiniDumpFilename(), 

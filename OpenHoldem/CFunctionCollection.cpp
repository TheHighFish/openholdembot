...
COHScriptObject *CFunctionCollection::LookUp(CString name) {
  CSLock lock(m_critsec);
  write_log(theApp.p_preferences()->debug_formula(), "[CFunctionCollection] Lookup %s\n", name); 
  std::map<CString, COHScriptObject*>::iterator it; 
  it = _function_map.find(name); 
  if (it == _function_map.end()) {
    // Function or list does not exist
    write_log(theApp.p_preferences()->debug_formula(), "[CFunctionCollection] Function does not exist\n");
    return NULL;
  }
  return it->second;
}

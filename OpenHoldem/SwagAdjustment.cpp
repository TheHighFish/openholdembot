...
double MinimumBetsizeDueToPreviousRaise() {
	...
	if (minimums_swag_amount <= 0)	{
		if (SYM->p_symbol_engine_tablelimits()->bblind() > 0) 	{
			write_log(MAIN->p_preferences()->debug_betsize_adjustment(), "[SwagAdjustment] MinimumBetsizeDueToPreviousRaise: set to 1 big-blind\n");
			minimums_swag_amount = SYM->p_symbol_engine_tablelimits()->bblind();
		}	else {
			write_log(MAIN->p_preferences()->debug_betsize_adjustment(), "[SwagAdjustment] MinimumBetsizeDueToPreviousRaise: SERIOUS TABLEMAP-PROBLEM; no bets; big-blind unknown; setting it to 0.02\n");
			// Setting it to the absolute minimum
			minimums_swag_amount = 0.02;
		}
	}
	write_log(MAIN->p_preferences()->debug_betsize_adjustment(), "[SwagAdjustment] MinimumBetsizeDueToPreviousRaise: %.2f\n", minimums_swag_amount);
...
double MaximumPossibleBetsizeBecauseOfBalance() {
	...
	write_log(MAIN->p_preferences()->debug_betsize_adjustment(), "[SwagAdjustment] MaximumPossibleBetsizeBecauseOfBalance %.2f\n",
		maximum_betsize);
...
double SwagAmountAjustedToCasino(double amount_to_raise_to) {
	...
	write_log(MAIN->p_preferences()->debug_betsize_adjustment(), "[SwagAdjustment] SwagAmountAjustedToCasino %.2f\n",
...
bool BetSizeIsAllin(double amount_to_raise_to_in_dollars_and_cents) {
	...
	write_log(MAIN->p_preferences()->debug_betsize_adjustment(), "[SwagAdjustment] BetSizeIsAllin() desired betsize: %.2f\n",
		amount_to_raise_to_in_dollars_and_cents);
	write_log(MAIN->p_preferences()->debug_betsize_adjustment(), "[SwagAdjustment] BetSizeIsAllin() maximum betsize: %.2f\n",
		maximum_betsize);
...
double RoundedBetsizeForTournaments(double amount_to_raise_to_in_dollars_and_cents) {
	if (!SYM->p_symbol_engine_istournament()->istournament()) {
		write_log(MAIN->p_preferences()->debug_betsize_adjustment(), 

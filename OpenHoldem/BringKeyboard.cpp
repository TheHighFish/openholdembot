...
void CheckBringKeyboard(void) {
	...
	if (!SYM->p_symbol_engine_casino()->ConnectedToBring()) 	{
		write_log(theApp.p_preferences()->debug_autoplayer(), "[BringKeyBoard] Not connected to bring, therefore no bring-keyboard to be enabled.\n");
		return;
	}

	write_log(theApp.p_preferences()->debug_autoplayer(), "[BringKeyBoard] Connected to bring.\n");
	write_log(theApp.p_preferences()->debug_autoplayer(), "[BringKeyBoard] Enabling bring-keyboard if necessary.\n");

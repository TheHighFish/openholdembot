...
void CSymbolEngineAutoplayer::CalculateMyTurnBits() {
	...
	for (int i=0; i<k_max_number_of_buttons; i++) {
      ...
      if (CStringMatch::IsStringFold(button_label))	{
				_myturnbits |= (1<<0);
			}	else if (CStringMatch::IsStringCall(button_label)) 	{
				_myturnbits |= (1<<1);
			}	else if (CStringMatch::IsStringRaise(button_label) || button_label.MakeLower() == "swag")	{
				_myturnbits |= (1<<2);
			}	else if (CStringMatch::IsStringCheck(button_label))	{
				_myturnbits |= (1<<4);
			}	else if (CStringMatch::IsStringAllin(button_label)) {
				_myturnbits |= (1<<3);
			}	else if (CStringMatch::IsStringAutopost(button_label)) 	{
				_isautopost = true;
			}
		}
	}
...
void CSymbolEngineAutoplayer::CalculateSitInState() {
  for (int i=0; i<k_max_number_of_buttons; i++) {
    if (CStringMatch::IsStringSitin(p_table_state->_SCI._button_label[i])) {
	    // Sitin-button found

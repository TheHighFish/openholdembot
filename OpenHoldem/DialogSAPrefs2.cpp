...
BOOL CDlgSAPrefs2::OnInitDialog()
{
	...
	text.Format("%d", theApp.p_preferences()->frame_delay());
	m_FrameDelay.SetWindowText(text);
	m_FrameDelay_Spin.SetRange(0, 9);
	m_FrameDelay_Spin.SetPos(theApp.p_preferences()->frame_delay());
	m_FrameDelay_Spin.SetBuddy(&m_FrameDelay);

	text.Format("%d", theApp.p_preferences()->click_delay());
	m_ClickDelay.SetWindowText(text);
	m_ClickDelay_Spin.SetRange(0, 5000);
	m_ClickDelay_Spin.SetPos(theApp.p_preferences()->click_delay());
	m_ClickDelay_Spin.SetBuddy(&m_ClickDelay);

	text.Format("%d", theApp.p_preferences()->swag_delay_1());
	m_SwagDelay1.SetWindowText(text);
	m_SwagDelay1_Spin.SetRange(0, 5000);
	m_SwagDelay1_Spin.SetPos(theApp.p_preferences()->swag_delay_1());
	m_SwagDelay1_Spin.SetBuddy(&m_SwagDelay1);

	text.Format("%d", theApp.p_preferences()->swag_delay_2());
	m_SwagDelay2.SetWindowText(text);
	m_SwagDelay2_Spin.SetRange(0, 5000);
	m_SwagDelay2_Spin.SetPos(theApp.p_preferences()->swag_delay_2());
	m_SwagDelay2_Spin.SetBuddy(&m_SwagDelay2);

	text.Format("%d", theApp.p_preferences()->swag_delay_3());
	m_SwagDelay3.SetWindowText(text);
	m_SwagDelay3_Spin.SetRange(0, 5000);
	m_SwagDelay3_Spin.SetPos(theApp.p_preferences()->swag_delay_3());
	m_SwagDelay3_Spin.SetBuddy(&m_SwagDelay3);

	m_Autoplayer_Upon_Connection.SetCheck(theApp.p_preferences()->engage_autoplayer() ? BST_CHECKED : BST_UNCHECKED);
	m_Use_Auto_Replay.SetCheck(theApp.p_preferences()->use_auto_replay() ? BST_CHECKED : BST_UNCHECKED);
	m_SwagUseComma.SetCheck(theApp.p_preferences()->swag_use_comma() ? BST_CHECKED : BST_UNCHECKED);
...
void CDlgSAPrefs2::OnOK()
{
	...
	theApp.p_preferences()->SetValue(k_prefs_frame_delay, strtoul(text.GetString(), 0, 10));
	...
	theApp.p_preferences()->SetValue(k_prefs_click_delay, strtoul(text.GetString(), 0, 10));
	...
	theApp.p_preferences()->SetValue(k_prefs_swag_delay_1, strtoul(text.GetString(), 0, 10));
	...
	theApp.p_preferences()->SetValue(k_prefs_swag_delay_2, strtoul(text.GetString(), 0, 10));
	...
	theApp.p_preferences()->SetValue(k_prefs_swag_delay_3, strtoul(text.GetString(), 0, 10));

	theApp.p_preferences()->SetValue(k_prefs_engage_autoplayer, m_Autoplayer_Upon_Connection.GetCheck()==BST_CHECKED ? true : false);
	theApp.p_preferences()->SetValue(k_prefs_use_auto_replay, m_Use_Auto_Replay.GetCheck()==BST_CHECKED ? true : false);
	theApp.p_preferences()->SetValue(k_prefs_swag_use_comma, m_SwagUseComma.GetCheck()==BST_CHECKED ? true : false);

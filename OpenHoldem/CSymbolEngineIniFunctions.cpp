...
#define RETURN_IF_LOADING_NEW_FORMULA if ((p_engine_container->p_function_collection() == NULL) || ((p_formula_parser != NULL) && (p_formula_parser->IsParsing()))) return;
...
void CSymbolEngineIniFunctions::InitOnStartup() {
  ...
  write_log(theApp.p_preferences()->debug_symbolengine(), "[Symbolengine] CSymbolEngineIniFunctions::InitOnStartup()\n");
  ResetOnConnection();
...
void CSymbolEngineIniFunctions::ResetOnConnection() {
  ...
  write_log(theApp.p_preferences()->debug_symbolengine(), "[Symbolengine] CSymbolEngineIniFunctions::ResetOnConnection()\n");
  SYM->p_function_collection()->Evaluate(k_standard_function_names[k_init_on_connection]);
  write_log(theApp.p_preferences()->debug_symbolengine(), "[Symbolengine] CSymbolEngineIniFunctions::ResetOnConnection() completed\n");
}

void CSymbolEngineIniFunctions::ResetOnHandreset(){
  RETURN_IF_LOADING_NEW_FORMULA

  write_log(theApp.p_preferences()->debug_symbolengine(), "[Symbolengine] CSymbolEngineIniFunctions::ResetOnHandreset()\n");
...
void CSymbolEngineIniFunctions::ResetOnNewRound() {
  ...
  write_log(theApp.p_preferences()->debug_symbolengine(), "[Symbolengine] CSymbolEngineIniFunctions::ResetOnNewRound()\n");
...
void CSymbolEngineIniFunctions::ResetOnMyTurn() {
  ...
  write_log(theApp.p_preferences()->debug_symbolengine(), "[Symbolengine] CSymbolEngineIniFunctions::ResetOnMyTurn()\n");
...
void CSymbolEngineIniFunctions::ResetOnHeartbeat() {
  ...
  write_log(theApp.p_preferences()->debug_symbolengine(), "[Symbolengine] CSymbolEngineIniFunctions::ResetOnHeartbeat()\n");
  write_log(theApp.p_preferences()->debug_symbolengine(), "[Symbolengine] CSymbolEngineIniFunctions::ResetOnHeartbeat() evaluating %s\n",

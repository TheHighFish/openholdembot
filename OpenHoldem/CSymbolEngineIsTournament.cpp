...
void CSymbolEngineIsTournament::TryToDetectTournament() {
	if (_istournament != k_undefined) {
		write_log(theApp.p_preferences()->debug_istournament(), "[CSymbolEngineIsTournament] Currently istournament = %s\n", Bool2CString(_istournament));
  } else {
		write_log(theApp.p_preferences()->debug_istournament(), "[CSymbolEngineIsTournament] Currently istournament = unknown\n");
  }
	// Don't do anything if we are already sure.
	// Don't mix things up.
	if (_decision_locked) {
		write_log(theApp.p_preferences()->debug_istournament(), "[CSymbolEngineIsTournament] decision already locked\n");
		return;
	}
	...
	if ((_istournament != k_undefined)
		...
		write_log(theApp.p_preferences()->debug_istournament(), "[CSymbolEngineIsTournament] Enough hands played; locking current value\n");
  ...
  if (SYM->p_symbol_engine_casino()->ConnectedToDDPoker()) {
    write_log(theApp.p_preferences()->debug_istournament(), "[CSymbolEngineIsTournament] DDPoker tournament\n");
    _istournament    = true;
		_decision_locked = true;
		return;
  }
  ...
  // http://www.maxinmontreal.com/forums/viewtopic.php?f=294&t=17625&start=30#p125608
	if (TitleStringContainsIdentifier(k_tournament_identifiers,
      k_number_of_tournament_identifiers))	{
		write_log(theApp.p_preferences()->debug_istournament(), "[CSymbolEngineIsTournament] Table title looks like a tournament\n");
		_istournament    = true;
		_decision_locked = true;
		return;
	}
  ...
  if (SYM->p_symbol_engine_active_dealt_playing()->nplayersplaying() < 2) {
    write_log(theApp.p_preferences()->debug_istournament(), "[CSymbolEngineIsTournament] Can't consider the blinds -- too few people playing.\n");
    return;
  }
  double bigblind = SYM->p_symbol_engine_tablelimits()->bblind();
	if ((bigblind > 0) && (bigblind < k_lowest_bigblind_ever_seen_in_tournament)) {
	  write_log(theApp.p_preferences()->debug_istournament(), "[CSymbolEngineIsTournament] Blinds \"too low\"; this is a cash-game\n");
	  _istournament    = false;
		_decision_locked = true;
		return;
  }
  ...
  if (SYM->p_symbol_engine_casino()->ConnectedToManualMode()) {
		write_log(theApp.p_preferences()->debug_istournament(), "[CSymbolEngineIsTournament] ManualMode, but no tournament identifier\n");
    return;
  }
	// If there are antes then it is a tournament.
	if (AntesPresent())	{
		write_log(theApp.p_preferences()->debug_istournament(), "[CSymbolEngineIsTournament] Game with antes; therefore tournament\n");
		_istournament    = true;
		_decision_locked = true;
		return;
	}
	...
	if (BetsAndBalancesAreTournamentLike())	{
		write_log(theApp.p_preferences()->debug_istournament(), "[CSymbolEngineIsTournament] Bets and balances look like tournament\n");
		_istournament    = true;
		_decision_locked = true;
		return;
	}
	...
	if (bigblind > k_large_bigblind_probably_later_table_in_tournament) {
		...
		write_log(theApp.p_preferences()->debug_istournament(), "[CSymbolEngineIsTournament] bigblind very large; probably tournament\n");
		_istournament = true;
		return;
	}
	...
	_istournament    = false;
	_decision_locked = true;
	write_log(theApp.p_preferences()->debug_istournament(), "[CSymbolEngineIsTournament] high-stakes cash-game up to $200\\$400\n");

...
CTableMapLoader::CTableMapLoader() {
	write_log(theApp.p_preferences()->debug_tablemap_loader(), "[CTablemapLoader] CTableMapLoader()\n");
...
void CTableMapLoader::ParseAllTableMapsToLoadConnectionData(CString TableMapWildcard) {
	...
	write_log(theApp.p_preferences()->debug_tablemap_loader(), "[CTablemapLoader] ParseAllTableMapsToLoadConnectionData: %s\n", TableMapWildcard);
	...
	while (bFound) {
		if (_number_of_tablemaps_loaded >= k_max_nmber_of_tablemaps) {
			write_log(theApp.p_preferences()->debug_tablemap_loader(), "[CTablemapLoader] CAutoConnector: Error: Too many tablemaps. The autoconnector can only handle 25 TMs.", "Error", 0);
        ...
        tablemap_completeness_checker.VerifyMap();
				write_log(theApp.p_preferences()->debug_tablemap_loader(), "[CTablemapLoader] Number of TMs loaded: %d\n", _number_of_tablemaps_loaded);
			}
		}
	}
}
...
void CTableMapLoader::ParseAllTableMapsToLoadConnectionData() {
	...
	write_log(theApp.p_preferences()->debug_tablemap_loader(), "[CTablemapLoader] ParseAllTableMapsToLoadConnectionData\n");
	_number_of_tablemaps_loaded = 0;
  ...
  assert(TablemapFilePath != "");
    ...
    assert(tablemap_connection_data[i].FilePath != "");
		if (tablemap_connection_data[i].FilePath == TablemapFilePath)	{
			write_log(theApp.p_preferences()->debug_tablemap_loader(), "[CTablemapLoader] tablemap_connection_dataAlreadyStored [%s] [true]\n", TablemapFilePath);
			return true;
		}
	}
	write_log(theApp.p_preferences()->debug_tablemap_loader(), "[CTablemapLoader] tablemap_connection_dataAlreadyStored [%s] [false]\n", TablemapFilePath);
	return false;
}
...
void CTableMapLoader::CheckForDuplicatedTablemaps() {
	...
	for (int i=0; i<_number_of_tablemaps_loaded; i++)
	{
		for (int j=i+1; j<_number_of_tablemaps_loaded; j++)
		{
			...
			{
				write_log(theApp.p_preferences()->debug_tablemap_loader(), "[CTablemapLoader] tablemap_connection_dataDuplicated [%s, %s]  [true]\n", 
...
void CTableMapLoader::ExtractConnectionDataFromCurrentTablemap(CTablemap *cmap) {
	write_log(theApp.p_preferences()->debug_tablemap_loader(), "[CTablemapLoader] ExtractConnectionDataFromCurrentTablemap(): %s\n", cmap->filepath());
	write_log(theApp.p_preferences()->debug_tablemap_loader(), "[CTablemapLoader] number_of_tablemaps_loaded: %d\n", _number_of_tablemaps_loaded);
	...
	if (tablemap_connection_dataAlreadyStored(cmap->filepath())) 	{
		write_log(theApp.p_preferences()->debug_tablemap_loader(), "[CTablemapLoader] ExtractConnectionDataFromCurrentTablemap(): already stored; early exit\n");
		return;
	}
...
bool Check_TM_Against_Single_Window(int MapIndex, HWND h, RECT r, CString title) {
  ...
  bool      good_table_points = false;
	...
	write_log(theApp.p_preferences()->debug_tablemap_loader(), "[CTablemapLoader] Check_TM_Against_Single_Window(..)\n");
	write_log(theApp.p_preferences()->debug_tablemap_loader(), "[CTablemapLoader] Checking map nr. %d\n", MapIndex);
	write_log(theApp.p_preferences()->debug_tablemap_loader(), "[CTablemapLoader] Window title: %s\n", title);
	...
	if (!((r.right == tablemap_connection_data[MapIndex].ClientSizeX)
		...
		if (!((tablemap_connection_data[MapIndex].ClientSizeMinX != 0) 
			...
			write_log(theApp.p_preferences()->debug_tablemap_loader(), "[CTablemapLoader] No good size: Expected (%dpx, %dpx), Got (%dpx, %dpx)\n",
	...
	write_log(theApp.p_preferences()->debug_tablemap_loader(), "[CTablemapLoader] Size matches; checking the rest...\n");
  ...
  // Check for match positive title text matches
	...
	if (!good_pos_title)
	{
		write_log(theApp.p_preferences()->debug_tablemap_loader(), "[CTablemapLoader] No good title.\n");
		return false;
	}
	...
	if (good_neg_title)
	{
		write_log(theApp.p_preferences()->debug_tablemap_loader(), "[CTablemapLoader] Negative title found -> window is no match.\n"); 
		return false;
	}
  ...
  if (tablemap_connection_data[MapIndex].TablePointCount > 0) {
    for (int i=0; i<tablemap_connection_data[MapIndex].TablePointCount; i++) {
      ...
      if (!good_table_points) {
         write_log(theApp.p_preferences()->debug_tablemap_loader(), "[CTablemapLoader] Not all tablepoints match.\n");
         return false;
      }
    }
	}

	write_log(theApp.p_preferences()->debug_tablemap_loader(), "[CTablemapLoader] Window ia a match\n");
	return true;
}
...
void CTableMapLoader::ReloadAllTablemapsIfChanged() {
	if (_p_filesystem_monitor->AnyChanges())	{		
		...
		write_log(theApp.p_preferences()->debug_tablemap_loader(), "[CTablemapLoader] Tablemaps changed; going to reload.\n");
		p_autoconnector->Disconnect();
		ParseAllTableMapsToLoadConnectionData();
	}	else {
		write_log(theApp.p_preferences()->debug_tablemap_loader(), "[CTablemapLoader] All tablemaps unchanged; nothing to do.\n");
	}
}

...
CHandresetDetector::CHandresetDetector() {
  write_log(theApp.p_preferences()->debug_handreset_detector(), "[CHandresetDetector] Executing constructor\n");
...
void CHandresetDetector::CalculateIsHandreset() {
  ...
  write_log(theApp.p_preferences()->debug_handreset_detector(), "[CHandresetDetector] Methods firing last 3 heartbeat2: %s\n",
    IntToBinaryString(total_methods_firing));
  int number_of_methods_firing = bitcount(total_methods_firing);
  write_log(theApp.p_preferences()->debug_handreset_detector(), "[CHandresetDetector] Number of methods firing last 3 heartbeat2: %i\n",
    number_of_methods_firing);
  if (number_of_methods_firing >= 2) {
    write_log(theApp.p_preferences()->debug_handreset_detector(), "[CHandresetDetector] Handreset found\n");
    ...
    write_log(theApp.p_preferences()->debug_handreset_detector(), "[CHandresetDetector] No handreset\n");
    _is_handreset_on_this_heartbeat = false;
  }
}
...
int CHandresetDetector::BitVectorFiringHandresetMethods() {
  ...
  write_log(theApp.p_preferences()->debug_handreset_detector(), "[CHandresetDetector] Methods firing this heartbeat: %s\n",
...
bool CHandresetDetector::IsHandresetByDealerChair() {
	...
	write_log(theApp.p_preferences()->debug_handreset_detector(), "[CHandresetDetector] Handreset by dealerchair: %s\n",
...
bool CHandresetDetector::IsHandresetByUserCards() {
	...
	write_log(theApp.p_preferences()->debug_handreset_detector(), "[CHandresetDetector] Handreset by cards: %s\n",
		Bool2CString(ishandreset));
	return ishandreset;
}
...
bool CHandresetDetector::IsHandresetByHandNumber() {
	...
	write_log(theApp.p_preferences()->debug_handreset_detector(), "[CHandresetDetector] Handreset by handnumber: %s\n",
		Bool2CString(ishandreset));
	return ishandreset;
}
...
bool CHandresetDetector::IsHandresetByCommunityCards() {
  bool ishandreset = ((_community_cards < _last_community_cards)
    && (_community_cards == 0));
  write_log(theApp.p_preferences()->debug_handreset_detector(), "[CHandresetDetector] Handreset by community cards: %s\n",
		Bool2CString(ishandreset));
  return ishandreset;
}
...
bool CHandresetDetector::IsHandresetByPotsize() {
  ...
  bool ishandreset = (_potsize < _last_potsize)
    ...
    && (_community_cards == 0);
  write_log(theApp.p_preferences()->debug_handreset_detector(), "[CHandresetDetector] Handreset by potsize: %s\n",
		Bool2CString(ishandreset));
  return ishandreset;
}
...
bool CHandresetDetector::IsHandresetByNopponentsplaying() {
  bool ishandreset = (_nopponentsplaying > _last_nopponentsplaying)
    && (_community_cards == 0);
  write_log(theApp.p_preferences()->debug_handreset_detector(), "[CHandresetDetector] Handreset by nopponentsplaying: %s\n",
		Bool2CString(ishandreset));
  return ishandreset;
}
...
bool CHandresetDetector::IsHandresetByIncreasingBalance() {
  ...
  write_log(theApp.p_preferences()->debug_handreset_detector(), "[CHandresetDetector] Handreset by increasing balance: %s\n",
		Bool2CString(ishandreset));
  return ishandreset;
}
...
bool CHandresetDetector::IsHandresetByNewSmallBlind() {
  bool ishandreset = SmallBlindExists() 
    ...
    && (_community_cards == 0);
  write_log(theApp.p_preferences()->debug_handreset_detector(), "[CHandresetDetector] Handreset by new small blind: %s\n",
		Bool2CString(ishandreset));
  return ishandreset;
}
...
bool CHandresetDetector::IsHandresetByChangingBlindLevel() {
  bool ishandreset = (_bblind != _last_bblind)
    && (_bblind != 0)
    && (_last_bblind != 0);
  write_log(theApp.p_preferences()->debug_handreset_detector(), "[CHandresetDetector] Handreset by changing blind level: %s\n",
		Bool2CString(ishandreset));
  return ishandreset;
}
...
void CHandresetDetector::GetNewSymbolValues() {
	...
	if (IsValidDealerChair(SYM->p_symbol_engine_dealerchair()->dealerchair()))	{
		...
		write_log(theApp.p_preferences()->debug_handreset_detector(), "[CHandresetDetector] Setting new dealerchair to [%i]\n", dealerchair);
	}
	if (IsValidHandNumber(p_table_state->_s_limit_info.handnumber()))	{
		write_log(theApp.p_preferences()->debug_handreset_detector(), "[CHandresetDetector] Setting handnumber to [%s]\n", handnumber);
		handnumber = p_table_state->_s_limit_info.handnumber();	
	}	else {
		write_log(theApp.p_preferences()->debug_handreset_detector(), "[CHandresetDetector] Setting handnumber to [%s] was skipped. Reason: [digits number not in range]\n", handnumber);
	}
...
void CHandresetDetector::StoreOldValuesForComparisonOnNextHeartbeat() {
	...
	write_log(theApp.p_preferences()->debug_handreset_detector(), "[CHandresetDetector] Setting last dealerchair to [%i]\n", dealerchair);

	...
	if (pmyframe->show_regions)
	{
		...
		for (i=0; i<N; i++)
		{
			if ( (i==dragged_region && dragging) || (i==drawrect_region && drawing_rect && drawing_started) )
			{
				...
				pDC->Rectangle(pDoc->tablemap.r$[i].left-1, pDoc->tablemap.r$[i].top-1, pDoc->tablemap.r$[i].right+1, pDoc->tablemap.r$[i].bottom+1);
...
void COpenScrapeView::OnLButtonDown(UINT nFlags, CPoint point)
{
	...
	if (drawing_rect)
	{
		...
		{
			if (pDoc->tablemap.r$[i].name == sel)
			{
				drawrect_region = i;

				// Update internal structure
				pDoc->tablemap.r$[drawrect_region].left = point.x;
				pDoc->tablemap.r$[drawrect_region].top = point.y;
				pDoc->tablemap.r$[drawrect_region].right = point.x;
				pDoc->tablemap.r$[drawrect_region].bottom = point.y;
				...
				Invalidate(false);
				i = (int) pDoc->tablemap.r$.GetSize() + 1;
	...
	else
	{
		...
		if (nFlags & MK_SHIFT)
		{
			for (i=0; i<pDoc->tablemap.r$.GetSize(); i++)
			{
				if (point.x >= (LONG) pDoc->tablemap.r$[i].left-1 &&
					point.x <= (LONG) pDoc->tablemap.r$[i].right+1 &&
					point.y >= (LONG) pDoc->tablemap.r$[i].top-1 &&
					point.y <= (LONG) pDoc->tablemap.r$[i].bottom+1 &&
					(theApp.m_TableMapDlg->m_TableMapTree.GetSelectedItem() == NULL || pDoc->tablemap.r$[i].name == sel) )
				{
					dragging = true;
					dragged_region = i;
					drag_left_offset = point.x - pDoc->tablemap.r$[i].left;
					drag_top_offset = point.y - pDoc->tablemap.r$[i].top;
					Invalidate(false);
					i = (int) pDoc->tablemap.r$.GetSize() + 1;
				}
			}
		...
		else
		{
			for (i=0; i<pDoc->tablemap.r$.GetSize(); i++)
			{
					...
					pDoc->tablemap.r$[i].name != sel)
				{
					...
					{
						if (pDoc->tablemap.r$[i].name == theApp.m_TableMapDlg->m_TableMapTree.GetItemText(temp_hti))  found = true;
...
void COpenScrapeView::OnLButtonUp(UINT nFlags, CPoint point)
{
	...
	if (drawing_rect)
	{
		...
		pDoc->tablemap.r$[drawrect_region].left = drawrect_start.x<point.x ? drawrect_start.x : point.x;
		pDoc->tablemap.r$[drawrect_region].top = drawrect_start.y<point.y ? drawrect_start.y : point.y;
		pDoc->tablemap.r$[drawrect_region].right = drawrect_start.x>=point.x ? drawrect_start.x : point.x;
		pDoc->tablemap.r$[drawrect_region].bottom = drawrect_start.y>=point.y ? drawrect_start.y : point.y;
...
void COpenScrapeView::OnMouseMove(UINT nFlags, CPoint point)
{
	...
	if (drawing_rect && drawing_started)
	{
		...
		// Update table map dialog
		text.Format("%d", pDoc->tablemap.r$[drawrect_region].left);
		theApp.m_TableMapDlg->m_Left.SetWindowText(text.GetString());
		text.Format("%d", pDoc->tablemap.r$[drawrect_region].top);
		theApp.m_TableMapDlg->m_Top.SetWindowText(text.GetString());
		text.Format("%d", pDoc->tablemap.r$[drawrect_region].right);
		theApp.m_TableMapDlg->m_Right.SetWindowText(text.GetString());
		text.Format("%d", pDoc->tablemap.r$[drawrect_region].bottom);
		theApp.m_TableMapDlg->m_Bottom.SetWindowText(text.GetString());
	...
	{
		width = pDoc->tablemap.r$[dragged_region].right - pDoc->tablemap.r$[dragged_region].left;
		height = pDoc->tablemap.r$[dragged_region].bottom - pDoc->tablemap.r$[dragged_region].top;

		// Update internal structure for selected region
		pDoc->tablemap.r$[dragged_region].left = point.x - drag_left_offset;
		pDoc->tablemap.r$[dragged_region].top = point.y - drag_top_offset;
		pDoc->tablemap.r$[dragged_region].right = pDoc->tablemap.r$[dragged_region].left + width;
		pDoc->tablemap.r$[dragged_region].bottom = pDoc->tablemap.r$[dragged_region].top + height;

		// Update table map dialog
		text.Format("%d", pDoc->tablemap.r$[dragged_region].left);
		theApp.m_TableMapDlg->m_Left.SetWindowText(text.GetString());
		text.Format("%d", pDoc->tablemap.r$[dragged_region].top);
		theApp.m_TableMapDlg->m_Top.SetWindowText(text.GetString());
		text.Format("%d", pDoc->tablemap.r$[dragged_region].right);
		theApp.m_TableMapDlg->m_Right.SetWindowText(text.GetString());
		text.Format("%d", pDoc->tablemap.r$[dragged_region].bottom);
		theApp.m_TableMapDlg->m_Bottom.SetWindowText(text.GetString());
...
void COpenScrapeView::blink_rect(void)
{
	...
	if (!dragging)
	{
		for (i=0; i<(int) pDoc->tablemap.r$.GetSize(); i++)
		{
			if (pDoc->tablemap.r$[i].name == sel)
			{
				pDC->Rectangle(pDoc->tablemap.r$[i].left-1, pDoc->tablemap.r$[i].top-1, pDoc->tablemap.r$[i].right+1, pDoc->tablemap.r$[i].bottom+1);

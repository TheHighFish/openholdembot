...
void CScraper::ScrapeName(int chair) {
	...
	CString Token, temp;
	int Position;
	CString sregex = "([0-9.,c$£€]+$)";
	CString errmsg = "Pattern error(s) on your regex expression in your unwanted string entries.\nPlease review it on Preferences->Scraper->Unwanted player names scraping.";
	...
	if (result != "") {
		...
		while (!Token.IsEmpty())
		{
			...
			//Handle all exact matches, fold, check, call raise, bet, allin, etc...
			if (Token == temp && Token != "raiseto" && Token != "time:") { return; }

			// Handle all matches for, call, raise, raiseto, bet, and allin with an amount.
			// This also handle time: and time  because sregex includes any digit 
			if (Token == "call" || Token == "raise" || Token == "raiseto" || Token == "bet" || Token == "time:") {
				if (regex_match((string)temp, regex(Token + sregex))) { return; }
			}

			// Handle for regex expression, advanced users must only put regex arguments
			try {

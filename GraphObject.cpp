...
void CGraphObject::OnPaint() 
{
	...
	else
	{
		...
		if ( m_Update == TRUE )
		{
			...
			oldBkColor = m_GraphDC->SetBkColor(m_GraphBackgroundColor);
			oldTextColor = m_GraphDC->SetTextColor(m_GraphAlertColor);
			//pOldFont = m_GraphDC->SelectObject(m_GraphAlertFont);		// For 1st version of SetGraphAlert() otherwise comment it
			m_GraphDC->SetBkMode(TRANSPARENT);
			//if (m_GraphAlert == TRUE)		// For 1st version of SetGraphAlert() otherwise comment it
			...
			if (m_GraphAlertString.GetLength()>10)
				...
				m_GraphDC->TextOut(int(m_Size.cx*0.52), int(m_Size.cy*0.28), m_GraphAlertString, m_GraphAlertString.GetLength());			
			m_GraphDC->SetBkMode(OPAQUE);
			m_GraphDC->SelectObject(pOldFont);
			m_GraphDC->SetBkColor(oldBkColor);
			m_GraphDC->SetTextColor(oldTextColor);
			...
			m_GraphDC->SetBkMode(TRANSPARENT);
			m_GraphDC->TextOut(int(m_Size.cx*0.52), int(m_Size.cy*0.38), m_GraphAlertLine, m_GraphAlertLine.GetLength());
			m_GraphDC->SetBkMode(OPAQUE);
			m_GraphDC->SelectObject(pOldFont);
			m_GraphDC->SetBkColor(oldBkColor);
			m_GraphDC->SetTextColor(oldTextColor);

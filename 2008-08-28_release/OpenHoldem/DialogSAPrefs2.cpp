...
BOOL CDlgSAPrefs2::OnInitDialog()
{
    ...
    text.Format("%d", p_global->preferences.frame_delay);
    m_FrameDelay.SetWindowText(text);
    m_FrameDelay_Spin.SetRange(0, 9);
    m_FrameDelay_Spin.SetPos(p_global->preferences.frame_delay);
    m_FrameDelay_Spin.SetBuddy(&m_FrameDelay);

    text.Format("%d", p_global->preferences.swag_delay_1);
    m_SwagDelay1.SetWindowText(text);
    m_SwagDelay1_Spin.SetRange(0, 5000);
    m_SwagDelay1_Spin.SetPos(p_global->preferences.swag_delay_1);
    m_SwagDelay1_Spin.SetBuddy(&m_SwagDelay1);

    text.Format("%d", p_global->preferences.swag_delay_2);
    m_SwagDelay2.SetWindowText(text);
    m_SwagDelay2_Spin.SetRange(0, 5000);
    m_SwagDelay2_Spin.SetPos(p_global->preferences.swag_delay_2);
    m_SwagDelay2_Spin.SetBuddy(&m_SwagDelay2);

    text.Format("%d", p_global->preferences.swag_delay_3);
    m_SwagDelay3.SetWindowText(text);
    m_SwagDelay3_Spin.SetRange(0, 5000);
    m_SwagDelay3_Spin.SetPos(p_global->preferences.swag_delay_3);
    ...
    if (p_global->preferences.button_click_method == BUTTON_SINGLECLICK)
        m_ButtonClicking.SelectString(0, "Single");
    else if (p_global->preferences.button_click_method == BUTTON_DOUBLECLICK)
        m_ButtonClicking.SelectString(0, "Double");

    m_AutoAP.SetCheck(p_global->preferences.ap_auto ? BST_CHECKED : BST_UNCHECKED);

    m_FocusDetect.SetCheck(p_global->preferences.focus_detect ? BST_CHECKED : BST_UNCHECKED);
...
void CDlgSAPrefs2::OnOK()
{
    ...
    p_global->preferences.frame_delay = strtoul(text.GetString(), 0, 10);
    ...
    p_global->preferences.swag_delay_1 = strtoul(text.GetString(), 0, 10);
    ...
    p_global->preferences.swag_delay_2 = strtoul(text.GetString(), 0, 10);
    ...
    p_global->preferences.swag_delay_3 = strtoul(text.GetString(), 0, 10);

    if (m_TextSelection.GetCurSel()==0)
        p_global->preferences.text_selection_method = TEXTSEL_DOUBLECLICK;
    else if (m_TextSelection.GetCurSel()==1)
        p_global->preferences.text_selection_method = TEXTSEL_CLICKDRAG;

    if (m_TextDeletion.GetCurSel()==0)
        p_global->preferences.text_deletion_method = TEXTDEL_DELETE;
    else if (m_TextDeletion.GetCurSel()==1)
        p_global->preferences.text_deletion_method = TEXTDEL_BACKSPACE;

    if (m_BetConfirmation.GetCurSel()==0)
        p_global->preferences.bet_confirmation_method = BETCONF_ENTER;
    else if (m_BetConfirmation.GetCurSel()==1)
        p_global->preferences.bet_confirmation_method = BETCONF_CLICKBET;

    if (m_ButtonClicking.GetCurSel()==0)
        p_global->preferences.button_click_method = BUTTON_SINGLECLICK;
    else if (m_ButtonClicking.GetCurSel()==1)
        p_global->preferences.button_click_method = BUTTON_DOUBLECLICK;

    p_global->preferences.ap_auto = m_AutoAP.GetCheck()==BST_CHECKED ? true : false;
    p_global->preferences.focus_detect = m_FocusDetect.GetCheck()==BST_CHECKED ? true : false;

    reg.read_reg();
    reg.frame_delay = p_global->preferences.frame_delay;
    reg.swag_delay_1 = p_global->preferences.swag_delay_1;
    reg.swag_delay_2 = p_global->preferences.swag_delay_2;
    reg.swag_delay_3 = p_global->preferences.swag_delay_3;
    reg.text_selection_method = p_global->preferences.text_selection_method;
    reg.text_deletion_method = p_global->preferences.text_deletion_method;
    reg.bet_confirmation_method = p_global->preferences.bet_confirmation_method;
    reg.button_click_method = p_global->preferences.button_click_method;
    reg.ap_auto = p_global->preferences.ap_auto;
    reg.focus_detect = p_global->preferences.focus_detect;

...
void CReplayFrame::CreateReplayFrame(void)
{
	...
	EnterCriticalSection(&p_heartbeat_thread->cs_update_in_progress);
		...
		// Create bitmap file
		path.Format("%s\\replay\\session_%lu\\frame%06d.bmp", _startup_path, theApp._session_id, _next_replay_frame);
		CreateBMPFile(path.GetString(), p_scraper->entire_window_cur());

		// Create HTML file
		path.Format("%s\\replay\\session_%lu\\frame%06d.htm", _startup_path, theApp._session_id, _next_replay_frame);
